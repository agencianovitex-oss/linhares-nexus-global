...
const PORTRAIT_MAP: Record<string, { src: string; position: string }> = {
  andre: { src: andreLinharesPortrait, position: "center 20%" },
  nicholas: { src: nicholas3, position: "center 18%" },
  juliana: { src: juliana, position: "center 20%" },
};

const PUB_IMAGES = [pubEb2Asset.url, pubInvestAsset.url, pubExecAsset.url];

/* ------------------------------------------------------------------ */
/* 1 — Hero                                                            */
/* ------------------------------------------------------------------ */

function ItemIcon({ kind }: { kind: HomeContent["authority"]["overview"]["items"][number]["icon"] }) {
  const cls = "h-[18px] w-[18px] text-gold shrink-0 mt-[3px]";
  switch (kind) {
    case "landmark": return <Landmark className={cls} strokeWidth={1.4} />;
    case "flag": return <Flag className={cls} strokeWidth={1.4} />;
    case "scale": return <Scale className={cls} strokeWidth={1.4} />;
    case "trophy": return <Trophy className={cls} strokeWidth={1.4} />;
    case "star": return <Star className={cls} strokeWidth={1.4} />;
  }
}

function AuthorityPanel({ c }: { c: HomeContent }) {
  const sequence = [...c.authority.slides.map((s) => ({ ...s, kind: "highlight" as const })), { ...c.authority.overview, kind: "overview" as const, id: "overview" }];
  ...
  }, [index, cycleKey, sequence.length]);

  const goTo = (target: number) => {
    setVisible(false);
    setTimeout(() => {
      setIndex(((target % sequence.length) + sequence.length) % sequence.length);
      setVisible(true);
      setCycleKey((k) => k + 1);
    }, 250);
  };
  ...
  const portrait = !isOverview && "portrait" in slide && slide.portrait ? PORTRAIT_MAP[slide.portrait] : undefined;
  const icon = !isOverview && "icon" in slide ? slide.icon : undefined;
  ...
  return (
    <div className="relative w-full max-w-xl mx-auto lg:mx-0 lg:ml-auto">
      <div
        ...
        style={{ background: "radial-gradient(ellipse 85% 75% at 50% 40%, oklch(1 0 0 / 0.06), transparent 70%)" }}
      />
      <div className="relative px-8 py-10 lg:px-10 lg:py-12 h-[560px] md:h-[600px] lg:h-[660px] overflow-hidden">
        {isFeatured && (
          <>
            <span aria-hidden className="pointer-events-none absolute inset-x-8 top-0 h-px bg-gold/80" />
            <span aria-hidden className="pointer-events-none absolute inset-0" style={{ background: "radial-gradient(ellipse 60% 45% at 50% 22%, rgb(179 134 66 / 0.18), transparent 70%)" }} />
          </>
        )}

        <div key={slide.id} className={`relative transition-all duration-[1100ms] ease-out ${visible ? "opacity-100 translate-y-0 blur-0" : "opacity-0 translate-y-3 blur-[2px]"}`}>
          <div className="flex justify-center">
            {portrait ? (
              ...
              <div
                ...
                style={{ background: "radial-gradient(circle at 50% 35%, oklch(1 0 0 / 0.09), oklch(1 0 0 / 0.02))", boxShadow: "0 0 0 5px oklch(1 0 0 / 0.04), 0 24px 50px -22px rgb(0 0 0 / 0.55)" }}
              >
                {icon === "award" ? <Award className="h-11 w-11 text-gold" strokeWidth={1.25} /> : icon === "star" ? <Star className="h-11 w-11 text-gold" strokeWidth={1.25} /> : <Scale className="h-11 w-11 text-gold" strokeWidth={1.25} />}
              </div>
            )}
          </div>

          <div className={`${isFeatured ? "mt-8 text-[12px] tracking-[0.36em]" : "mt-7 text-[11px] tracking-[0.32em]"} text-center font-bold uppercase text-gold`}>
            {slide.eyebrow}
          </div>

          {"name" in slide && slide.name && !isOverview && (
            <div className={`${isFeatured ? "mt-4 font-display text-[1.4rem] lg:text-[1.55rem] tracking-wide text-primary-foreground" : "mt-3 font-display text-[1.05rem] tracking-wide text-primary-foreground/90"} text-center`}>
              {slide.name}
            </div>
          )}

          <h2 className={`mt-3 text-center font-display font-semibold text-primary-foreground leading-[1.15] ${isFeatured ? "text-[1.85rem] lg:text-[2.15rem]" : isOverview ? "text-[1.35rem] lg:text-[1.55rem]" : "text-[1.5rem] lg:text-[1.75rem]"}`}>
            {slide.title}
          </h2>

          {isOverview ? (
            <ul className="mt-5 mx-auto max-w-md space-y-2.5">
              {c.authority.overview.items.map((it, i) => (
                <li key={i} className="flex items-start gap-3 text-[0.9rem] leading-[1.5] text-primary-foreground/85">
            ...
            <p className={`${isFeatured ? "mt-6 text-[1.02rem] leading-[1.8] text-primary-foreground/85" : "mt-5 text-[0.98rem] leading-[1.75] text-primary-foreground/75"} text-center max-w-md mx-auto`}>
              {"description" in slide ? slide.description : ""}
            </p>
          )}
        </div>

        <button type="button" aria-label={c.hero.prevSlide} onClick={prev} className="absolute left-1 top-1/2 -translate-y-1/2 h-9 w-9 rounded-full flex items-center justify-center text-primary-foreground/60 hover:text-gold hover:bg-primary-foreground/5 transition-colors">
          <ChevronLeft className="h-5 w-5" strokeWidth={1.5} />
        </button>
        <button type="button" aria-label={c.hero.nextSlide} onClick={next} className="absolute right-1 top-1/2 -translate-y-1/2 h-9 w-9 rounded-full flex items-center justify-center text-primary-foreground/60 hover:text-gold hover:bg-primary-foreground/5 transition-colors">
          <ChevronRight className="h-5 w-5" strokeWidth={1.5} />
        </button>

        <div className="absolute bottom-3 left-0 right-0 flex justify-center gap-2">
          {sequence.map((s, i) => (
            <button key={s.id} type="button" aria-label={c.hero.goToSlide(s.title)} onClick={() => goTo(i)} className={`h-[3px] rounded-full transition-all duration-500 ${i === index ? "w-10 bg-gold" : "w-5 bg-primary-foreground/25 hover:bg-primary-foreground/50"}`} />
          ))}
        </div>
      </div>
    </div>
        ...
        <div className="relative grid gap-16 lg:grid-cols-[minmax(0,1.05fr)_minmax(0,0.95fr)] lg:gap-12 lg:items-center">
          <div className="text-center lg:text-left">
            <div className="flex items-center justify-center lg:justify-start gap-4">
              ...
              <span className="eyebrow eyebrow-on-dark">{c.hero.eyebrow}</span>
            </div>
            <h1 className="mt-10 text-primary-foreground max-w-[60ch] mx-auto lg:mx-0">{c.hero.h1}</h1>
            <p className="mt-8 max-w-2xl mx-auto lg:mx-0 text-lg leading-[1.85] text-primary-foreground/80">{c.hero.sub}</p>
...
function AuthoritySection({ c }: { c: HomeContent }) {
  return (
    <section className="relative overflow-hidden bg-[rgb(6_36_67)] texture-grain pt-0 pb-20 lg:section-y">
      ...
      <div aria-hidden className="pointer-events-none absolute inset-0 hidden lg:block" style={{ backgroundImage: `url(${andrePracticeBg.url})`, backgroundSize: "cover", backgroundPosition: "left center", backgroundRepeat: "no-repeat" }}>
        <div className="absolute inset-0" style={{ background: "linear-gradient(90deg, rgb(6 36 67 / 0.15) 0%, rgb(6 36 67 / 0.25) 40%, rgb(6 36 67 / 0.7) 70%, rgb(6 36 67 / 0.88) 100%)" }} />
      </div>
      <div className="relative lg:hidden">
        <div className="relative h-[58vh] min-h-[400px] max-h-[540px] w-full overflow-hidden">
          <img src={andreLinharesPortrait} alt="Dr. André Linhares" className="absolute inset-0 h-full w-full object-cover object-[50%_top]" />
          <div aria-hidden className="absolute inset-0" style={{ background: "linear-gradient(180deg, rgb(6 36 67 / 0.25) 0%, rgb(6 36 67 / 0.15) 45%, rgb(6 36 67 / 0.85) 82%, rgb(6 36 67) 100%)" }} />
        </div>
      </div>
      <Container>
        <div className="relative grid gap-12 lg:grid-cols-12 lg:gap-24 items-center lg:min-h-[520px]">
          ...
          <div className="lg:col-span-7 lg:pt-8 relative -mt-10 lg:mt-0">
            <div className="flex items-center gap-4">
              <span className="rule-gold" />
              <span className="eyebrow eyebrow-on-dark">{c.practice.eyebrow}</span>
            </div>
            <h2 className="mt-7 text-balance text-primary-foreground max-w-[22ch]">{c.practice.h2}</h2>
            <p className="mt-8 lg:mt-10 text-base lg:text-lg leading-[1.85] text-primary-foreground/80 max-w-[58ch]">{c.practice.p1}</p>
            <p className="mt-6 text-base lg:text-lg leading-[1.85] text-primary-foreground/75 max-w-[58ch]">{c.practice.p2}</p>
            <div className="mt-10 flex items-center gap-4">
              <span className="rule-gold" />
              <span className="text-[10.5px] uppercase tracking-[0.28em] text-primary-foreground/60">{c.practice.author}</span>
            </div>
          </div>
        </div>
      </Container>
...
function AwardsSection({ c, localeHref }: { c: HomeContent; localeHref: (p: string) => string }) {
  return (
      ...
      <Container>
        <div className="grid gap-10 lg:grid-cols-12 lg:gap-16 items-end">
          <div className="lg:col-span-5">
            <div className="flex items-center gap-4"><span className="rule-gold" /><span className="eyebrow eyebrow-on-dark">{c.awards.eyebrow}</span></div>
            <h2 className="mt-7 text-balance text-primary-foreground max-w-[22ch]">{c.awards.h2}</h2>
          </div>
          <div className="lg:col-span-7">
            <p className="text-lg leading-[1.8] text-primary-foreground/75 max-w-[58ch]">{c.awards.sub}</p>
          </div>
        </div>
        ...
        <div className="mt-16 grid gap-12 lg:gap-20 lg:grid-cols-12 items-start">
          <div className="lg:col-span-5 relative">
            <div className="relative">
              ...
              <div className="mt-6">
                ...
                <h3 className="mt-4 text-2xl lg:text-[1.75rem] font-light text-primary-foreground tracking-tight leading-[1.25] max-w-[26ch]">{c.awards.ibiTitle}</h3>
              </div>
            </div>
          </div>
          ...
          <div className="lg:col-span-7 lg:pt-4">
            <div className="border-t border-primary-foreground/15">
              {c.awards.items.map((r) => (
                <div key={r.title} className="grid grid-cols-12 gap-6 py-8 lg:py-10 border-b border-primary-foreground/15 items-baseline">
                  <div className="col-span-12 sm:col-span-3 font-display text-2xl lg:text-3xl font-light text-gold tracking-tight leading-none">{r.year}</div>
                  <div className="col-span-12 sm:col-span-9">
                    <h3 className="text-2xl lg:text-[1.6rem] font-light text-primary-foreground tracking-tight leading-[1.25]">{r.title}</h3>
                    <div className="mt-2 text-[11px] uppercase tracking-[0.24em] text-primary-foreground/55">{r.org}</div>
                    <p className="mt-3 text-base leading-[1.7] text-primary-foreground/75 max-w-[48ch]">{r.desc}</p>
                  </div>
                </div>
              ))}
            </div>
...
function ServicesSection({ c, localeHref, visasBase }: { c: HomeContent; localeHref: (p: string) => string; visasBase: string }) {
  const featured = c.services.strategies[0];
  const rest = c.services.strategies.slice(1);
  return (
    <section className="section-y surface-premium-surface relative">
      ...
      <Container>
        <div className="flex flex-col lg:flex-row lg:items-end lg:justify-between gap-10">
          <SectionTitle eyebrow={c.services.eyebrow} title={c.services.title} description={c.services.description} />
          <InstitutionalButton to={localeHref(visasBase)} variant="primary" className="bg-gold text-white border-gold hover:bg-[rgb(200,145,70)] hover:text-white">
            {c.services.ctaAll}
          </InstitutionalButton>
        </div>
        ...
        <div className="mt-16 grid gap-px bg-border lg:grid-cols-3 border border-border">
          <Link to={`${localeHref(visasBase)}/${featured.slug}`} className="group editorial-card relative bg-primary text-primary-foreground p-10 lg:p-14 lg:col-span-2 lg:row-span-2 flex flex-col justify-between min-h-[460px] overflow-hidden">
            ...
            <div>
              ...
              <div className="mt-10 font-display font-semibold text-primary-foreground tracking-[-0.04em] leading-[0.9] text-[clamp(4rem,9vw,7.5rem)]">{featured.code}</div>
              <div className="mt-4 text-[11px] uppercase tracking-[0.32em] text-gold">{featured.title}</div>
              <p className="mt-8 max-w-xl text-base lg:text-lg leading-[1.8] text-primary-foreground/80">{featured.desc}</p>
            </div>
            <div className="mt-12 flex items-center justify-between gap-6 border-t border-primary-foreground/20 pt-6">
              <span className="text-[11px] uppercase tracking-[0.28em] text-primary-foreground/65">{featured.profile}</span>
              <span className="text-[11px] uppercase tracking-[0.28em] text-gold group-hover:translate-x-1 transition-transform">{c.services.learn}</span>
            </div>
          </Link>
          ...
          {rest.map((v) => (
            <Link key={v.slug} to={`${localeHref(visasBase)}/${v.slug}`} className="group editorial-card relative bg-gold text-white p-8 lg:p-10 flex flex-col justify-between min-h-[260px] hover:bg-[rgb(200,145,70)] overflow-hidden">
              ...
              <div className="mt-6 flex items-center justify-between gap-4 border-t border-white/25 pt-5">
                <span className="text-[10px] uppercase tracking-[0.28em] text-white/70">{v.profile}</span>
                <span className="text-[10.5px] uppercase tracking-[0.28em] text-white group-hover:translate-x-1 transition-transform">→</span>
              </div>
            </Link>
          ))}
        </div>
...
function LeadershipSection({ c, localeHref }: { c: HomeContent; localeHref: (p: string) => string }) {
  const founder = c.leadership.founder;
  const others = c.leadership.others.map((m, i) => ({ ...m, img: i === 0 ? nicholas3 : juliana }));
  return (
    <section className="section-y surface-premium-light relative">
      <span className="section-seam absolute top-0 left-0 right-0" aria-hidden />
      <Container>
        <div className="flex flex-col lg:flex-row lg:items-end lg:justify-between gap-10">
          <SectionTitle eyebrow={c.leadership.eyebrow} title={c.leadership.title} description={c.leadership.description} />
...
function ThoughtLeadershipSection({ c }: { c: HomeContent }) {
  return (
    <section className="section-y surface-premium-dark texture-grain relative overflow-hidden">
      <span className="fade-edge-top" aria-hidden />
      <span className="fade-edge-bottom" aria-hidden />
          ...
          <div className="lg:col-span-6 order-2 lg:order-1">
            <div className="relative">
              ...
              <div className="mt-5 flex items-center gap-4">
                <span className="rule-gold" />
                <span className="text-[10.5px] uppercase tracking-[0.28em] text-primary-foreground/60">{c.thought.caption}</span>
              </div>
            </div>
          </div>
          <div className="lg:col-span-6 order-1 lg:order-2">
            <div className="flex items-center gap-4"><span className="rule-gold" /><span className="eyebrow eyebrow-on-dark">{c.thought.eyebrow}</span></div>
            <h2 className="mt-7 text-balance text-primary-foreground max-w-[20ch]">{c.thought.h2}</h2>
            <p className="mt-7 text-lg leading-[1.8] text-primary-foreground/80 max-w-xl">{c.thought.sub}</p>
            <div className="mt-10 border-t border-primary-foreground/15">
              {c.thought.blocks.map((b) => (
...
function CultureSection({ c }: { c: HomeContent }) {
  return (
      ...
      <Container>
        <div className="grid gap-8 lg:grid-cols-12 lg:gap-10 items-stretch">
          <div className="order-1 lg:order-2 lg:col-span-5 flex flex-col justify-center">
            <div className="flex items-center gap-4"><span className="rule-gold" /><span className="eyebrow">{c.culture.eyebrow}</span></div>
            <h2 className="mt-5 text-balance text-primary max-w-[22ch] text-[clamp(1.625rem,2.4vw,2.25rem)] leading-[1.15] font-light">{c.culture.h2}</h2>
            <p className="mt-5 text-[15px] leading-[1.75] text-ink-soft max-w-[52ch]">{c.culture.sub}</p>
            <p className="mt-5 text-[10.5px] uppercase tracking-[0.28em] text-muted-foreground">{c.culture.dateLine}</p>
          </div>
          <div className="order-2 lg:order-3 lg:col-span-4 flex flex-col gap-px bg-border border border-border">
            <div className="bg-primary text-primary-foreground px-7 py-6 lg:px-8 lg:py-7 flex-1 flex flex-col justify-center">
              ...
              <div className="mt-3 text-[12.5px] leading-[1.55] text-primary-foreground/85 max-w-[28ch]">{c.culture.stat1}</div>
            </div>
            <div className="bg-gold text-gold-foreground px-7 py-6 lg:px-8 lg:py-7 flex-1 flex flex-col justify-center">
              <div className="font-display font-light tracking-[-0.03em] leading-none text-[clamp(3.5rem,5.5vw,5rem)]">100%</div>
              <div className="mt-3 text-[12.5px] leading-[1.55] text-gold-foreground/85 max-w-[28ch]">{c.culture.stat2}</div>
            </div>
          </div>
...
function OfficesSection({ c }: { c: HomeContent }) {
  return (
    <section className="relative surface-premium-dark overflow-hidden">
      <div aria-hidden className="absolute inset-0">
        <img src={skylineUs} alt="" aria-hidden loading="lazy" className="h-full w-full object-cover object-center opacity-[0.55]" />
        <div className="absolute inset-0" style={{ background: "linear-gradient(180deg, rgb(6 36 67 / 0.78) 0%, rgb(6 36 67 / 0.65) 50%, rgb(6 36 67 / 0.9) 100%)" }} />
      </div>
        ...
        <Container>
          <div className="grid gap-12 lg:grid-cols-12 lg:gap-16 items-end">
            <div className="lg:col-span-8">
              <div className="flex items-center gap-4"><span className="rule-gold" /><span className="eyebrow eyebrow-on-dark">{c.offices.eyebrow}</span></div>
              <h2 className="mt-7 text-balance text-primary-foreground max-w-[26ch]">{c.offices.h2}</h2>
              <p className="mt-7 max-w-2xl text-lg leading-[1.85] text-primary-foreground/75">{c.offices.sub}</p>
            </div>
          </div>
          <div className="mt-14 border-t border-primary-foreground/15">
            {c.offices.list.map((o, idx) => (
              <div key={o.city} className="grid grid-cols-12 gap-3 sm:gap-6 items-baseline py-5 sm:py-7 lg:py-8 border-b border-primary-foreground/15">
                <div className="col-span-2 sm:col-span-1 text-[10.5px] uppercase tracking-[0.32em] text-gold">0{idx + 1}</div>
                <div className="col-span-10 sm:col-span-4">
                  <div className="flex items-baseline justify-between gap-3 sm:block">
                    <div className="text-xl sm:text-2xl lg:text-[1.75rem] font-light text-primary-foreground tracking-tight leading-[1.1]">{o.city}</div>
                    <div className="sm:hidden text-[9.5px] uppercase tracking-[0.22em] text-primary-foreground/55 text-right shrink-0">{o.role}</div>
                  </div>
                  <div className="mt-1 flex items-baseline justify-between gap-3 sm:block">
                    <div className="text-[10.5px] uppercase tracking-[0.26em] text-primary-foreground/55">{o.state}</div>
                  </div>
                </div>
                <div className="hidden sm:block sm:col-span-4 text-[12px] uppercase tracking-[0.26em] text-primary-foreground/75">{o.role}</div>
                <div className="col-span-12 sm:col-span-3 sm:text-right text-[11px] uppercase tracking-[0.22em] text-primary-foreground/65 leading-snug">{o.address}</div>
              </div>
            ))}
          </div>
        </Container>
...
function PublicationsSection({ c, localeHref, locale }: { c: HomeContent; localeHref: (p: string) => string; locale: Locale }) {
  ...
  return (
    <section className="section-y relative overflow-hidden" style={{ backgroundColor: "rgb(179 134 66)" }}>
      <span className="section-seam absolute top-0 left-0 right-0" aria-hidden />
      <div aria-hidden className="pointer-events-none absolute inset-0" style={{ background: "linear-gradient(180deg, rgb(199 154 86) 0%, rgb(179 134 66) 50%, rgb(159 114 46) 100%)" }} />
      <Container>
        <div className="relative flex flex-col lg:flex-row lg:items-end lg:justify-between gap-10">
          <div>
            <div className="flex items-center gap-4">
              ...
              <span className="text-[11px] font-semibold uppercase tracking-[0.34em] text-white/85">{c.publications.eyebrowLead}</span>
            </div>
            <h2 className="mt-6 text-balance text-white max-w-[22ch]">{c.publications.h2}</h2>
            <p className="mt-5 max-w-2xl text-base lg:text-lg leading-[1.8] text-white/85">{c.publications.sub}</p>
          </div>
          <Link to={localeHref("/blog")} className="inline-flex items-center gap-3 px-7 py-4 text-[11px] font-semibold tracking-[0.26em] uppercase border border-white bg-white text-[rgb(6_36_67)] hover:bg-transparent hover:text-white transition-colors whitespace-nowrap">
            {c.publications.ctaAll} <span aria-hidden>→</span>
          </Link>
        </div>
            ...
            <Link key={i} to={localeHref("/blog")} className="group editorial-card bg-background flex flex-col overflow-hidden hover:bg-background">
              ...
              <div className="flex flex-col justify-between flex-1 p-8 lg:p-10 min-h-[220px]">
                <div>
                  ...
                  <h3 className="mt-5 text-xl lg:text-[1.4rem] font-light text-primary tracking-tight leading-[1.3] group-hover:text-gold transition-colors">{p.title}</h3>
                </div>
                <div className="mt-8 flex items-center justify-between border-t border-border pt-5">
                  <span className="text-[10.5px] uppercase tracking-[0.28em] text-muted-foreground">{p.meta}</span>
                  <span className="text-[10.5px] uppercase tracking-[0.28em] text-primary group-hover:text-gold transition-colors">{c.publications.readCta}</span>
                </div>
              </div>
            </Link>
          ))}
...
function FinalCTA({ c, localeHref }: { c: HomeContent; localeHref: (p: string) => string }) {
  return (
    <section className="section-y surface-premium-dark texture-grain relative overflow-hidden">
      <span className="fade-edge-top" aria-hidden />
      <div aria-hidden className="pointer-events-none absolute inset-0 opacity-[0.05]" style={{ backgroundImage: "radial-gradient(circle at 85% 50%, rgb(179 134 66 / 0.6), transparent 55%)" }} />
      <Container>
        <div className="relative grid gap-12 lg:grid-cols-12 lg:gap-20 items-center">
          <div className="lg:col-span-7">
            <div className="flex items-center gap-4"><span className="rule-gold" /><span className="eyebrow eyebrow-on-dark">{c.finalCTA.eyebrow}</span></div>
            <h2 className="mt-8 text-primary-foreground text-[clamp(2rem,4.5vw,3.75rem)] font-light leading-[1.05] tracking-tight max-w-[18ch]">{c.finalCTA.h2}</h2>
          </div>
          <div className="lg:col-span-5 lg:border-l lg:border-primary-foreground/15 lg:pl-12">
            <p className="text-lg leading-[1.8] text-primary-foreground/75">{c.finalCTA.sub}</p>
            <div className="mt-10">
              <InstitutionalButton to={localeHref("/contato")} variant="primary" className="bg-gold text-white border-gold hover:bg-[rgb(200,145,70)] hover:text-white">
                {c.finalCTA.cta}
              </InstitutionalButton>
...
export function Home() {
  ...
  const localeHref = (path: string) => withLocale(locale, path);
  ...
  return (
    <>
      ...
      <ServicesSection c={c} localeHref={localeHref} visasBase={visasBase} />
      <LeadershipSection c={c} localeHref={localeHref} />
