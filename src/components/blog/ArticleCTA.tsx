...
export function ArticleCTA(props: Props) {
  ...
  if (props.variant === "sidebar") {
    return (
      <aside className="bg-[rgb(6_36_67)] p-5 text-white">
        {c.title && <h3 className="font-display text-lg leading-tight">{c.title}</h3>}
        <p className="text-[13px] leading-relaxed text-white/75">{c.description}</p>
        <a href={c.url}
  ...
  return (
    <aside className="my-12 overflow-hidden rounded-xl bg-[rgb(6_36_67)] p-8 text-white sm:p-10">
      {c.title && <h3 className="font-display text-2xl leading-tight sm:text-3xl">{c.title}</h3>}
      <p className={c.title ? "mt-3 text-white/80" : "text-white/80"}>{c.description}</p>
