...
export function AuthorCard({ author, locale }: Props) {
  ...
  return (
    <section className="my-12 flex flex-col gap-5 rounded-xl border border-border/50 bg-card p-6 sm:flex-row sm:items-start sm:p-8">
      {author.photo_url ? (
        <img loading="lazy" decoding="async" src={mediaUrl(author.photo_url)} alt={author.name} className="h-20 w-20 flex-shrink-0 rounded-full object-cover" />
      ...
      <div className="space-y-2">
        ...
        <Link {...taxonomyLink(locale, "author", author.slug)} className="block font-display text-xl text-[rgb(6_36_67)] hover:underline">
          {author.name}
        </Link>
