...
const L = "es" as const;
const KIND = "tag" as const;
const search = z.object({ page: z.coerce.number().int().min(1).optional() });

export const Route = createFileRoute("/_site/es/blog/tag/$slug")({
  ...
  head: ({ loaderData, params }) => buildLocaleHead({
    path: `/blog/tag/${params.slug}`, locale: L,
    title: `${loaderData?.name ?? params.slug}, ${tBlog(L).tag}, Linhares Law`,
